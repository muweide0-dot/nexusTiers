...
const pointRules = [
  ...
  { tier: 2, highCode: 'HT2', high: 30, lowCode: 'LT2', low: 20 },
  { tier: 3, highCode: 'HT3', high: 10, lowCode: 'LT3', low: 6 },
  { tier: 4, highCode: 'HT4', high: 4, lowCode: 'LT4', low: 3 },
  { tier: 5, highCode: 'HT5', high: 2, lowCode: 'LT5', low: 1 },
] as const;
...
function tierLabel(value?: string) {
  ...
  return normalized && /^(?:ht|lt)[1-5]$/.test(normalized) ? normalized.toUpperCase() : '—';
}
...
function Overview() {
  ...
  return <div className="animate-rise">
    ...
    <div className="mt-8 grid gap-3 md:grid-cols-[1fr_auto]"><div className="rounded-xl border border-border bg-card/70 p-4"><div className="flex items-center gap-3"><Server className="h-4 w-4 text-primary" /><div><p className="text-sm font-medium">Minecraft network bridge</p><p className="mt-0.5 text-xs text-muted-foreground">Discord queue relay · Railway service · live updates enabled</p></div><Badge tone="green" >healthy</Badge></div></div><Link href="/players" className="focus-ring flex items-center justify-center gap-2 rounded-xl border border-border bg-card/70 px-5 py-4 text-sm font-medium hover:bg-secondary" data-testid="link-player-directory">Player directory <ArrowRight className="h-4 w-4 text-primary" /></Link></div>
  </div>;
}
...
function MinecraftHead({ ign, size = 'h-10 w-10' }: { ign: string; size?: string }) {
  const [failed, setFailed] = useState(false);
  if (failed || !ign) return <Avatar name={ign || '?'} hue="hsl(191 83% 55%)" />;
  return <img src={'https://mc-heads.net/avatar/' + encodeURIComponent(ign) + '/64'} alt={ign + ' Minecraft head'} className={size + ' rounded-md border border-white/10 bg-secondary object-cover'} loading="lazy" onError={() => setFailed(true)} />;
}

function PodiumCard({ row }: { row: LeaderboardRow }) {
  const RankIcon = row.rank === 1 ? Crown : row.rank === 2 ? Medal : Trophy;
  return <div className={'podium-card podium-card-' + row.rank + ' rounded-xl border p-4'}><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-3"><MinecraftHead ign={row.ign} size="h-12 w-12" /><div className="min-w-0"><p className="truncate text-base font-semibold">{row.ign}</p><p className="truncate text-xs text-muted-foreground">{row.username}</p></div></div><span className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-background/50 text-accent"><RankIcon className="h-4 w-4" /></span></div><div className="mt-5 flex items-end justify-between"><div><p className="font-mono text-[10px] uppercase tracking-[0.14em] text-muted-foreground">Rank #{String(row.rank).padStart(2, '0')}</p><p className="mt-1 tier-pill">{tierLabel(row.bestTier)}</p></div><div className="text-right"><span className="stat-number text-3xl font-semibold text-accent">{row.points}</span><span className="ml-1 font-mono text-[10px] uppercase text-muted-foreground">points</span></div></div></div>;
}

function latestTierResultsByKit<T extends { kit: string; createdAt: string }>(results: T[]) {
  const latest = new Map<string, T>();
  for (const result of results) {
    const key = result.kit.toLowerCase();
    const current = latest.get(key);
    if (!current || new Date(result.createdAt).getTime() > new Date(current.createdAt).getTime()) {
      latest.set(key, result);
    }
  }
  return Array.from(latest.values()).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
}
function PlayerProfileDialog({ ign, onClose }: { ign: string; onClose: () => void }) {
  const profile = useGetPlayerTiers(ign || 'lookup', { query: { enabled: Boolean(ign), queryKey: getGetPlayerTiersQueryKey(ign || 'lookup') } });
  const leaderboard = useLeaderboard();
  if (!ign) return null;
  const data = profile.data;
  const row = leaderboard.data?.find((candidate) => candidate.ign.toLowerCase() === ign.toLowerCase());
  const position = row?.rank ?? null;
  const points = row?.points ?? 0;
  const badge = points >= 300 ? 'Combat Master' : points >= 100 ? 'Combat Specialist' : 'Combat Rookie';
  const results = latestTierResultsByKit(data?.tiers ?? []);

  return <div className="profile-modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <div className="profile-modal" role="dialog" aria-modal="true" aria-label={ign + ' profile'}>
      <button type="button" className="profile-modal-close" onClick={onClose} aria-label="Close profile"><X className="h-6 w-6" /></button>
      {profile.isLoading ? <LoadingState label={'Loading ' + ign} /> : profile.isError || !data ? <ErrorState message="This player profile is temporarily unavailable." /> : <div className="profile-modal-content">
        <MinecraftHead ign={data.ign} size="h-28 w-28" />
        <h2 className="profile-modal-name">{data.ign}</h2>
        <div className="profile-title-badge"><Crown className="h-5 w-5" />{badge}</div>
        <p className="profile-region">{data.username}</p>
        <div className="profile-section"><p className="profile-section-label">POSITION</p><div className="profile-position-row"><span className="profile-position-number">{position ? position + '.' : '—'}</span><span className="profile-position-title"><Trophy className="h-5 w-5 text-accent" />OVERALL</span><span className="profile-position-points">({points}<br />points)</span></div></div>
        <div className="profile-section"><p className="profile-section-label">TIERS</p><div className="profile-tier-strip">{results.length === 0 ? <span className="profile-empty">No tier results yet</span> : results.map((result) => <div className={'profile-tier-badge profile-tier-' + result.tier.slice(0, 2).toLowerCase()} key={result.id}><span className="profile-tier-icon"><Trophy className="h-4 w-4" /></span><span>{tierLabel(result.tier)}</span></div>)}</div></div>
      </div>}
    </div>
  </div>;
}
...
function LeaderboardPage() {
  ...
  return <div className="animate-rise">
    <div className="leaderboard-hero mb-8 overflow-hidden rounded-2xl border border-border p-6 md:p-9"><div className="relative z-[1] flex flex-wrap items-end justify-between gap-6"><div><div className="mb-4 flex items-center gap-2"><Badge tone="amber"><Trophy className="h-3 w-3" />all kits</Badge><span className="font-mono text-[10px] uppercase tracking-[0.18em] text-muted-foreground">one ranking</span></div><h1 className="max-w-2xl text-4xl font-semibold tracking-[-0.06em] md:text-6xl">Leaderboard</h1><p className="mt-4 max-w-xl text-sm leading-relaxed text-muted-foreground md:text-base">All kits, one ranking. HT1 is the best tier, LT5 is the lowest, and every verified result adds the matching points.</p></div><div className="leaderboard-hero-stat"><p className="font-mono text-[10px] uppercase tracking-[0.16em] text-muted-foreground">Ranked players</p><p className="mt-2 text-3xl font-semibold text-foreground">{rows.length}</p><p className="mt-1 text-xs text-muted-foreground">across all kits</p></div></div></div>
    <div className="kit-switcher mb-6 overflow-x-auto rounded-xl border border-border bg-card/60 p-2"><div className="flex min-w-max gap-2"><span className="flex items-center px-3 text-[10px] font-mono uppercase tracking-[0.12em] text-muted-foreground">View one kit:</span>{kits.map((kit) => <Link key={kit.key} href={'/rankings/' + kit.key} className="rounded-lg px-3 py-2 text-xs font-medium text-muted-foreground transition hover:bg-secondary hover:text-foreground"><span className="mr-2 font-mono text-[9px]" style={{ color: kit.hue }}>{kit.short}</span>{kit.label}</Link>)}</div></div>
     <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]"><section className="leaderboard-surface rounded-2xl border border-border bg-card/65 p-4 md:p-6"><div className="mb-5 flex items-end justify-between gap-4"><div><p className="mb-1 font-mono text-[10px] uppercase tracking-[0.18em] text-primary">01 / all kits</p><h2 className="text-xl font-semibold tracking-tight">Leaderboard</h2></div><span className="rounded-full border border-primary/20 bg-primary/10 px-2.5 py-1 font-mono text-[10px] text-primary">{rows.length} PLAYERS</span></div>{leaderboard.isLoading ? <LoadingState label="Loading leaderboard" /> : leaderboard.isError ? <ErrorState onRetry={() => leaderboard.refetch()} message="The leaderboard is temporarily unavailable." /> : rows.length === 0 ? <EmptyState icon={Trophy} title="No ranked players yet" description="Submit a verified result to create the first leaderboard entry." /> : <><div className="mb-6 grid gap-3 md:grid-cols-3">{rows.slice(0, 3).map((row) => <PodiumCard key={row.ign} row={row} />)}</div><div className="leaderboard-table overflow-hidden rounded-xl border border-border"><div className="leaderboard-row leaderboard-row-head"><span>Rank</span><span>Player</span><span>Kits</span><span>Best tier</span><span>Tests</span><span className="text-right">Points</span></div>{rows.map((row) => <div className="leaderboard-row" key={row.ign}><span className={'rank-number rank-' + row.rank}>{String(row.rank).padStart(2, '0')}</span><button type="button" className="profile-player-button flex min-w-0 items-center gap-3" onClick={() => setSelectedPlayer(row.ign)} aria-label={'Open profile for ' + row.ign}><MinecraftHead ign={row.ign} size="h-9 w-9" /><div className="min-w-0 text-left"><p className="truncate text-sm font-medium">{row.ign}</p><p className="truncate text-[11px] text-muted-foreground">{row.username}</p></div></button><div className="flex flex-wrap gap-1">{(row.kits ?? []).map((kit) => <span key={kit} className="rounded border border-border bg-secondary/70 px-1.5 py-0.5 font-mono text-[9px] uppercase text-muted-foreground">{kitMeta(kit).short}</span>)}</div><span className="tier-pill">{tierLabel(row.bestTier)}</span><span className="text-xs text-muted-foreground">{row.results} test{row.results === 1 ? '' : 's'}</span><span className="text-right font-mono text-sm font-semibold text-accent">{row.points} pts</span></div>)}</div></>}</section><aside className="space-y-5"><section className="points-card rounded-2xl border border-border bg-card/70 p-5 md:p-6"><div className="mb-5 flex items-start justify-between"><div><p className="mb-1 font-mono text-[10px] uppercase tracking-[0.18em] text-accent">02 / points</p><h2 className="text-xl font-semibold">Tier points</h2></div><TrendingUp className="h-5 w-5 text-accent" /></div><div className="space-y-2">{pointRules.map((rule) => <div className="point-rule" key={rule.tier}><div className="flex items-center gap-3"><span className="point-trophy"><Trophy className="h-3.5 w-3.5" /></span><span className="text-sm font-medium">Tier {rule.tier}</span></div><div className="flex items-center gap-2 font-mono text-[10px]"><span className="text-foreground">{rule.highCode} {rule.high}</span><span className="text-muted-foreground">{rule.lowCode} {rule.low}</span></div></div>)}</div><p className="mt-4 text-[11px] leading-relaxed text-muted-foreground">HT1 is the best rank. LT5 is the lowest rank. HT earns the first value; LT earns the second value.</p></section><section className="rounded-2xl border border-primary/15 bg-primary/5 p-5"><div className="flex items-center gap-2 text-primary"><ShieldCheck className="h-4 w-4" /><p className="text-sm font-medium">Earn points through testing</p></div><p className="mt-3 text-xs leading-relaxed text-muted-foreground">Complete a verified kit test and your best result is reflected here automatically.</p><Link href="/players" className="mt-4 inline-flex items-center gap-2 text-xs font-medium text-primary hover:underline">Find a player <ArrowRight className="h-3.5 w-3.5" /></Link></section></aside></div>
